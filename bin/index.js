...
async function main() {
  ...
  console.clear()
	...
	p.intro(`${picocolors.white(  'Welcome to branicons')}`);
  ...
  const replVariables = await p.confirm({
	...
	const colors = replVariables ? await p.group(
		{
			fColor: () => p.text({ message: "Type in the color to be replaced" }),
			rColor: ({results}) => p.text({ message: `What color should we replace ${picocolors.cyan(results.fColor)} with?` }),
		},
		{
  ...
  const outputFilePath = outputFileName + '.json'
		...
		files.forEach((file) => {
			...
			fs.stat(filePath, (err, stats) => {
				...
				if (stats.isFile()) {
					...
					var replPathStringsArray = replPathStringsArray.replace(colors.fColor, colors.rColor);
	...
	p.outro((`Icon set completed. You can find it here: ${picocolors.cyan(outputFilePath)}`));
}
